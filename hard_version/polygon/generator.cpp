...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < t; i++) {
        ...
        if (mode == "random") {
            int remaining_cases=n-i-1;
            n = rnd.next(1, (int)min(max_n,500000-(int)sum_n-t+1+i));
            k = rnd.next(1, max_k);
        ...
        sum_n += n;
        ensuref(sum_n <= 500000LL, "Sum of N exceeded 5*10^5");
        ensuref(1LL * k * l <= n, "K * L (%lld) exceeds N (%d)", 1LL * k * l, n);
